...
# ─────────────────────────────────────────────────────────────────────────────
#  SimVPv2-style spatiotemporal predictor
# ─────────────────────────────────────────────────────────────────────────────

class _SimVPSpatialBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 3, padding=1, bias=False),
...
class _SimVPTemporalBlock(nn.Module):
    def __init__(self, hidden: int):
        super().__init__()
        self.dw = nn.Conv3d(hidden, hidden, kernel_size=(3, 3, 3), padding=1, groups=hidden, bias=False)
        self.pw = nn.Conv3d(hidden, hidden, kernel_size=1, bias=False)
...
class SimVPv2Lite(nn.Module):
    """
    SimVPv2-style direct predictor for PM2.5.
    ...
    def __init__(
        ...
        enc = [_SimVPSpatialBlock(in_channels, hidden)]
        for _ in range(max(0, spatial_layers - 1)):
            enc.append(_SimVPSpatialBlock(hidden, hidden))
        self.encoder2d = nn.Sequential(*enc)

        self.temporal = nn.Sequential(*[_SimVPTemporalBlock(hidden) for _ in range(max(1, temporal_layers))])

        self.decoder2d = nn.Sequential(
            _SimVPSpatialBlock(hidden, hidden),
            nn.Conv2d(hidden, 1, kernel_size=1),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Expected SimVP layout: (B, T, C, H, W)
        # Accept legacy layout too: (B, C, T, H, W)
        if x.ndim != 5:
            raise ValueError(f"SimVP expects 5D input, got {x.shape}")

        if x.shape[1] != self.in_steps and x.shape[2] == self.in_steps:
            x = x.permute(0, 2, 1, 3, 4)

        B, T, C, H, W = x.shape
        if T != self.in_steps:
            raise ValueError(f"SimVP expected T_in={self.in_steps}, got T={T}")

        # Frame-wise 2D encoding
        z = x.reshape(B * T, C, H, W)
        z = self.encoder2d(z)
        hidden = z.shape[1]
        z = z.reshape(B, T, hidden, H, W).permute(0, 2, 1, 3, 4)  # (B, hidden, T, H, W)

        # Temporal mixing in 3D
        z = self.temporal(z)

        # If out_steps differs from in_steps, resize on temporal axis.
        if self.out_steps != self.in_steps:
            z = F.interpolate(z, size=(self.out_steps, H, W), mode='trilinear', align_corners=False)

        # Frame-wise decode to PM2.5 channel
        z = z.permute(0, 2, 1, 3, 4).reshape(B * self.out_steps, hidden, H, W)
        y = self.decoder2d(z)  # (B*T_out, 1, H, W)
        y = y.reshape(B, self.out_steps, 1, H, W)
...
def build_model(cfg) -> nn.Module:
    ...
    elif mtype in ('simvpv2', 'simvp'):
        model = MODEL_REGISTRY[mtype](
            in_channels = cfg['model'].get('simvp_in_channels', input_channels),
            in_steps = cfg['time'].get('t_in', 16),
            out_steps = t_out,
            hidden = cfg['model'].get('simvp_hidden', 96),
            spatial_layers = cfg['model'].get('simvp_spatial_layers', 3),
            temporal_layers = cfg['model'].get('simvp_temporal_layers', 4),
        )
